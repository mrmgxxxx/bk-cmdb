...
func (s *Service) ListSubscriptions(req *restful.Request, resp *restful.Response) {
	...
	if auth.EnableAuthorize() {
		...
		if err != nil {
			// 200, list authorized subscription resources failed.
			blog.ErrorJSON("list authorized subscriptions failed, err: %v, cond: %s, rid: %s", err, authInput, rid)
		...
		for _, resourceID := range authorizedResources {
			...
			if err != nil {
				// 200, parse resourceid failed.
				blog.Errorf("list authorized subscriptions parse resourceid[%s] failed, err: %v, rid: %s", resourceID, err, rid)
				resp.WriteError(http.StatusOK, &metadata.RespError{Msg: err})
				return
			}
...
func (s *Service) trimSubscriptionForm(subscriptionForm string) []string {
	...
	subscriptionForm := strings.Split(subscriptionFormStr, ",")

	sort.Strings(subscriptionForm)
	return strings.Join(subscriptionForm, ",")
}
