...
func (es *EventServer) initModules() error {
	...
	subWatcher, err := reflector.NewReflector(es.configs.Mongo.GetMongoConf())
	if err != nil {
	...
	// init distributer.
	distributer, err := distribution.NewDistributer(es.ctx, es.db, es.redisCli, es.subWatcher)
	if err != nil {
		return fmt.Errorf("create event distributer, %+v", err)
	}
...
func Run(ctx context.Context, cancel context.CancelFunc, op *options.ServerOption) error {
	...
	for {
		go func() {
			errCh <- distribution.SubscribeChannel(eventServer.SubCli())
		}()

		go func() {
			errCh <- distribution.Start(ctx, eventServer.DB(), eventServer.RedisCli(),
				eventServer.Engine().CoreAPI, eventServer.Engine().ServiceManageInterface)
		}()
	...
	if err := backbone.StartServer(ctx, cancel, eventServer.Engine(),
		eventServer.Service().WebService(), true); err != nil {
		return err
	}
	...
	case err := <-errCh:
		blog.Errorf("EventServer stopping now, the distribution coroutine stopped, %+v", err)
		return err
	}
}
